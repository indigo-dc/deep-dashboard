...
sed -i.bak -r 's/^(numprocs )(.*)$/# Commented out by '"$ME"' on '"$(date)"'\n#\1\2\n\1= '"$((2*$ncpu+1))"'/' /etc/supervisor/conf.d/aiohttp.conf

server_string=
for n in `seq $((2*$ncpu+1))`; do
  server_string="$server_string\n    server unix:\/tmp\/dashboard_$n.sock fail_timeout=0;"
done
